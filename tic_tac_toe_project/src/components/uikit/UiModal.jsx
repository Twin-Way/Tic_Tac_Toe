...
export default function UiModal({ width = 'md', className, children, isOpen = false, onClose }) {
  const handleClick = e => {
  ...
  const modal = (
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="w-8 h-8 flex items-center hover:bg-white/40 transition-colors justify-center bg-white/10 absolute top-0 left-[calc(100%+12px)] rounded"
        >
...
UiModal.Footer = function UiModalFooter({ children, className }) {
  return <div className={clsx(className, 'p-6 flex mt-auto gap-4 justify-end')}>{children}</div>;
};
